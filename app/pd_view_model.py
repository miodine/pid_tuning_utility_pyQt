...
class ViewModel(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init_event_connectors(self):
        self.radio_pitch.toggled.connect(lambda: self._ui_select_axis(self.AID['PITCH']))
        self.radio_roll.toggled.connect(lambda: self._ui_select_axis(self.AID['ROLL']))
        self.radio_yaw.toggled.connect(lambda: self._ui_select_axis(self.AID['YAW']))
        self.radio_alt.toggled.connect(lambda: self._ui_select_axis(self.AID['ALTITUDE']))

        self.button_upload_tuning.clicked.connect(self._upload_defined_controller_values)
    ...
    def _dynamic_plot_clear(self):
        ...
        self.dp_attitude        = self.dp_attitude[self.atp, self._n-1]*np.ones((3,self._n))
        self.dp_attitude_target = self.dp_attitude[self.atp, self._n-1]*np.ones((3,self._n))
